...
.word	def // CPSWCore0RxIsr   // 41  Ethernet core 0 rx dma completion
.word	def // CPSWCore0TxIsr   // 42  Ethernet core 0 tx dma completion
...
OLDCODE OLDCODE OLDCODE OLDCODE OLDCODE OLDCODE OLDCODE OLDCODE OLDCODE OLDCODE OLDCODE 
...
MASK_ACTIVE_IRQ         = INTC_SIR_IRQ_ACTIVEIRQ

INTC_CONTROL_NEWIRQAGR  = 0x1
NEWIRQAGR               = INTC_CONTROL_NEWIRQAGR

MODE_SYS                = 0x1F
MODE_IRQ                = 0x12
I_BIT                   = 0x80

ADDR_SIR_IRQ            = SOC_AINTC_REGS + INTC_SIR_IRQ
ADDR_CONTROL            = SOC_AINTC_REGS + INTC_CONTROL
ADDR_IRQ_PRIORITY       = SOC_AINTC_REGS + INTC_IRQ_PRIORITY
ADDR_THRESHOLD          = SOC_AINTC_REGS + INTC_THRESHOLD
...
1:  ldr	r1, [r_base, INTC_SYSSTATUS]
    ...
    mov r1, 0xff                       // PriorityThreshold
    str r1, [r_base, INTC_THRESHOLD]   // disable threshold spruh73l 6.5.1.11 
    dsb
...
// spruh73l 26.1.3.2 default boot procedure uses address 0x4030CE00
// for base of RAM exception vector.  see tab 26.3 for vector addresses
    ldr r_base, =0x4030CE24   // register UND in interrupt vector
    ldr r1, =und_isr
    str r1, [r_base]
    ldr r_base, =0x4030CE38   // register IRQ in interrupt vector
    ...
    ldr r_base, =0x4030CE00   // set base address of exception vector
    bl CP15VectorBaseAddrSet  // def 0x20000  spruh73n 26.1.3.1  // TODO DOES NOT WORK!
...
// my isr does not run prob cuz of the pukerware nested isr strategy  - TODO MUST FIX!!!
//
// IRQ interrupt service routine
//
// hacked from Al Selen, github.com/auselen
...
// Designed for very lean ISRs that are simple, fast and no loops.
.global irq_isr
irq_isr:                        // my original irq isr - no nesting no priority sorting
    stmfd sp!, {r0-r3, r12, lr} // must align stack to 8-byte boundary
    mrs r12, spsr               // save program status register

    mrs r1, cpsr
    orr r1, r1, #0x80  // disable IRQ   NOOB NOTE:  NO NEW IRQs WHILE PROCESSING AN IRQ!
    msr cpsr_c, r1                                  THEREFORE YOUR ISRs MUST BE FAST!
...
///////////////////////////////////above is me - below is TI 

//
// IRQ interrupt service routine   MODDED TI IRQ ISR      THIS WORKS ALSO
//
.global irq_isr
irq_isr:
...
//        AND      r2, r2, #MASK_ACTIVE_IRQ
        ...
        LDR      r0, =irq_vector          // Load the base of the vector table
...
//
// IRQ interrupt service routine     TI  IRQ ISR     THIS WORKS
//
.global irq_isr
irq_isr:  // this runs but should be cleaned up
        SUB      r14, r14, #4             // Apply lr correction
        STMFD    r13!, {r0-r3, r12, r14}  // Save context in IRQ stack
        MRS      r12, spsr                // Copy spsr
        STMFD    r13!, {r1, r12}          // Save fpscr and spsr
